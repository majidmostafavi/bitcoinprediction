...
from tensortrade.exchanges.live import BitmexExchange, BitmexExchange2, BitmexExchange3
...
from tensortrade.rewards import SimpleProfit, RiskAdjustedReturns, BitmexProfit, AdvancedProfit, NewProfit
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # sma50 = TAlibIndicator(indicators=[['SMA', {'args':['close'], 'params':{'timeperiod':2}}]])
        ...
        feature_pipeline = FeaturePipeline(steps=[normalized_price, normalized_volume, stochastic, macd])
        exchange = BitmexExchange3(data_frame=df, base_instrument='USDT', window_size=window_size, initial_balance=initial_balance, commission_percent=0.60, leverage=leverage, stop_loss_percent=stop_loss_percent)
        ...
        if train:
            for i in range(iteration):
                print('*-------------- iteration =', i+1, ' --------------*')
                try:
                    strategy.restore_agent(path="./tensortrade/agents/train_9", custom_objects=custom_objects)
                ...
                strategy.save_agent(path="./tensortrade/agents/train_9")
            ...
            strategy.restore_agent(path="./tensortrade/agents/train_9")
