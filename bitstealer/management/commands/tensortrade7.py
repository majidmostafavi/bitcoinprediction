...
from tensortrade.exchanges.live import BitmexExchange
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        feature_pipeline = FeaturePipeline(steps=[normalized_price, normalized_volume, ema, sma50, sma100, macd, stochastic, cci, rsi])
        exchange = BitmexExchange(data_frame=df, base_instrument='USDT', window_size=window_size, initial_balance=initial_balance, commission_percent=0.60, leverage=leverage, stop_loss_percent=stop_loss_percent)
        ...
        if train:
            for i in range(iteration):
                print('*-------------- iteration =', i+1, ' --------------*')
                try:
                    strategy.restore_agent(path="./tensortrade/agents/train_7", custom_objects=custom_objects)
                ...
                strategy.save_agent(path="./tensortrade/agents/train_7")
            ...
            strategy.restore_agent(path="./tensortrade/agents/train_7")
