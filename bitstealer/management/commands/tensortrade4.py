...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        feature_pipeline = FeaturePipeline(steps=[normalized_volume, normalized_price])
        ...
        model_kwargs = {'learning_rate': learning_rate, 'nminibatches': 1, 'gamma': gamma, 'lam': lam}
        ...
        strategy = BitmexTradingStrategySBL(environment=environment, model=DQN, policy='MlpPolicy')
        ...
        if train:
            for i in range(iteration):
                print('*-------------- iteration =', i+1, ' --------------*')
                try:
                    strategy.restore_agent(path="./tensortrade/agents/train_4", custom_objects=custom_objects)
