    ...
    def _make_trade(self, trade: Trade):
        ...
        if trade.is_long:
            ...
            if self.total_position >= 0:
                ...
                self.entry_price = ((self.entry_price * abs(self.total_position)) + (trade.price * trade.amount)) / (abs(self.total_position) + trade.amount)
                ...
                if trade.amount < abs(self.total_position):
                    self._balance += self.total_margin * (1 - commission) * (trade.amount / abs(self.total_position)) * (1 + plm)
