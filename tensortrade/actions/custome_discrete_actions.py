...
class CustomeDiscreteActions(ActionScheme):
    ...
    def __init__(self,
        ...
        super().__init__(action_space=MultiDiscrete([3, n_splitt]), dtype=np.int64)
    ...
    def get_trade(self, current_step: int, action: TradeActionUnion) -> Trade:
        trade_type = TradeType(action[0])
        trade_amount = (1 / self.n_splitt)*(float(action[1]) + 1)
