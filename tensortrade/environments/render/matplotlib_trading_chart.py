...
class MatplotlibTradingChart:
    ...
    def _render_trades(self, step_range, trades):
        for _, trade in trades.iterrows():
            if trade['step'] in range(sys.maxsize)[step_range]:
                ...
                if 'long' in str(trade['type']).lower():
                    color = 'green'

                elif 'short' in str(trade['type']).lower():
                    color = 'red'
    ...
    def render(self, current_step, net_worths, benchmarks, trades, window_size=250, total_position=0):
        net_worth = round(net_worths[-1], 4)
        initial_net_worth = round(net_worths[0], 4)
        profit_percent = round((net_worth - initial_net_worth) / initial_net_worth * 100, 2)
        total_position = round(total_position)

        self.fig.suptitle('Balance:' + str(net_worth) + ' XBT' +
                          ' | Profit: ' + str(profit_percent) + '%'
                          ' | Total Position: ' + str(total_position) + ' USDT'
                          )
