...
from mlagents.base_env import BaseEnv, AgentGroup, AgentGroupSpec, BatchedStepResult
...
class UnityEnvWorker:
    ...
    def send(self, name: str, payload: Any = None) -> None:
        try:
            ...
            raise Exception("worker: send failed.")
    ...
    def recv(self) -> EnvironmentResponse:
        try:
            ...
            raise Exception(" worker: recv failed.")
...
def worker(
    ...
    def _generate_all_results(action) -> AllStepResult:
        ...
        all_step_result[brain_name] = BatchedStepResult(obs=obs, reward=reward, done=done, max_step=max_step, agent_id=[0],action_mask=None)
    ...
    def external_brains():
        ...
        result[brain_name] = group_spec_to_brain_parameters(
            brain_name, {"observation_shapes": [(env.observation_space,)], "action_shape": (env.action_scheme.action_space.n,),
    ...
    def __init__(
        ...
        for worker_idx in range(n_env):
            self.env_workers.append(
                self.create_worker(
                    worker_idx, self.step_queue, env_factory
                )
            )
